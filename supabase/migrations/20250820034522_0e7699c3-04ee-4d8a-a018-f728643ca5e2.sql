-- Add status column to message_templates table for outreach stages
ALTER TABLE public.message_templates ADD COLUMN status text NOT NULL DEFAULT 'contacted';
...
ALTER COLUMN status TYPE template_status_enum USING status::template_status_enum;

-- Insert professional message templates for each stage/channel combination
